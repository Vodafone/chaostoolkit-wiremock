...
from chaoswm.probes import mappings
...
@unittest.skipIf(
    can_connect_to("localhost", 8080) is False,
...
class TestActions(unittest.TestCase):
    ...
    def test_wm_server_host_and_port(self):
        params = get_wm_params(
            {"wiremock": {"host": "localhost", "port": 8080, "timeout": 10}}
        )
    ...
    def test_global_random_delay(self):
        self.assertEqual(
            global_random_delay(
                {"type": "lognormal", "median": 20, "sigma": 0.1},
                {
                    "wiremock": {
                        "host": "localhost",
                        "port": 8080,
    ...
    def test_add_mappings(self):
        ...
        delete_mappings(
            filter=[{"url": "/some/thing", "method": "GET"}],
            configuration={
                "wiremock": {
                    "host": "localhost",
                    "port": 8080,
    ...
    def test_delete_mappings(self):
        self.assertEqual(
            reset(
                {
                    "wiremock": {"host": "localhost", "port": 8080},
                }
            ),
            1,
        )
        ...
        ids_deleted = delete_mappings(
            filter=[{"url": "/some/thing", "method": "GET"}],
            configuration={"wiremock": {"host": "localhost", "port": 8080}},
        )
    ...
    def test_fixed_delay(self):
        ...
        self.assertEqual(delayed[0], ids_added[0])
        ids_deleted = delete_mappings(
            filter=[{"url": "/some/thing", "method": "GET"}],
            configuration={"wiremock": {"host": "localhost", "port": 8080}},
        )
    ...
    def test_chunked_dribble_delay(self):
        ...
        self.assertEqual(delayed[0]["id"], ids_added[0])
        self.assertEqual(
            delayed[0]["response"]["chunkedDribbleDelay"]["numberOfChunks"], 5
        )
        ids_deleted = delete_mappings(
            filter=[{"url": "/some/thing", "method": "GET"}],
            configuration={"wiremock": {"host": "localhost", "port": 8080}},
    ...
    def test_down(self):
        ...
        delayed = down(
            filter=[{"method": "GET", "url": "/some/thing"}],
            configuration={
                "wiremock": {
                    "host": "localhost",
                    "port": 8080,
                    "defaults": {
                        "down": {"numberOfChunks": 10, "totalDuration": 4000}
                    },
                },
            },
        )
        self.assertEqual(delayed[0]["id"], ids_added[0])
        self.assertEqual(
            delayed[0]["response"]["chunkedDribbleDelay"]["numberOfChunks"], 10
        )
    ...
    def test_up(self):
        ...
        m = mappings({"wiremock": {"host": "localhost", "port": 8080}})
